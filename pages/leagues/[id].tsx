          ...
          {!data?.league ? (<>No league found!</>) :
            (
              <div>
                ...
                <TransfersForLeague loading={loading} playerTransfers={playerTransfers} />
              </div>
            )}
        </div>
      </div>
...
interface TransfersForLeagueProps {
  loading: boolean;
  playerTransfers: Map<string, EntryTransfer[]>;
}

function TransfersForLeague({ playerTransfers, loading }: TransfersForLeagueProps) {
  if (loading)
    return (
      <div className="flex justify-center">
        <Spinner size="lg" />
  ...
  playerTransfers.forEach((value: EntryTransfer[], key: string) => {
    items.push(
      (
        <div key={"abc" + key + value.length} className="divide-y-4 divide-fpl-purple">
